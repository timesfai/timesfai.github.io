...
LoginAction.Extends(BaseHttpAction, {
	...
	loginAction2:	function(doc) {
		...
		//if (doc.indexOf("低流量或手機版本") >= 0) {//登录失败
		//	postError("登录失败");
		//	this.end(ERROR);
		//	return;
		//}
...
CheckLoginAction.Extends(BaseHttpAction, {
	...
	action1:	function(doc) {
		...
		//if (doc.indexOf('<a href="dorf3.php" accesskey="9" title="村落概貌">村庄:</a>') >= 0) {
...
BuildAction.Extends(BaseHttpAction, {
	...
	buildAction1:	function(doc) {
		if (doc.indexOf('時後方會有足夠的資源') >= 0 || doc.indexOf('资源何时充足时间提示') >= 0) {
			postMessage("资源不足");
			this.end(NOT_READY);
			return;
		}
		if (doc.indexOf("所有工人都在忙碌") >= 0 || doc.indexOf("等待队列") >= 0) {
		...
		if (doc.indexOf("完全地開發") >= 0 || doc.indexOf("将马上开始全部建造") >= 0) {
			postError("建造完成");
			this.end(ERROR);
			return;
		}
		...
		info = getStrBetween(doc, '<h1 class="titleInHeader">', '</h1>', null);//<h1><b>农场 等级 3</b></h1>
...
BuildVillageAction.Extends(BaseHttpAction, {
	...
	action1:	function(doc) {
		...
		if (doc.indexOf("建築物：") >= 0) {
			...
			var name;
			while (true) {
				name  = getStrBetween(info, '<img src="img/x.gif" class="del" title="取消" alt="取消" /></a></td><td>', '</span></td>', null);//<table ...><tr><td>...title="取消"></a></td><td>市场 (等级 14)</td>...
				if (name == "")
					break;//not found
				var inner = true;
				if (name.indexOf("伐木場") >= 0 || name.indexOf("黏土矿") >= 0 || name.indexOf("泥坑") >= 0 || name.indexOf("鐵礦場") >= 0 || name.indexOf("農場") >= 0)
					inner = false;
...
AttackAction.Extends(BaseHttpAction, {
	...
	action1:	function(doc) {
		...
		for (property in troops) {//property = "t1"
			...
			//var info = startFromStr(doc, 'name="t'+property+'"', 2000);//t1 方阵
var info = startFromStr(doc, '<table id="troops" cellpadding="1" cellspacing="1">', 5000);//t1 方阵
			...
			postMessage("部队类型 "+property+" 数量:"+teminfo);
			//var iCount = parseDecimal( getStrBetween(info, ">", "<", null) );
		...
		info = startFromStr(vcdoc, 'name="timestamp"', 30);//t1 方阵
		var timestamp = getStrBetween(info, 'value="', '" />', null);

//postMessage("<textarea>" + doc +"</testarea>")
		info = startFromStr(vcdoc, 'name="timestamp_checksum"', 20);//t1 方阵
	...
	action2:	function(doc) {
		...
		var sAttackDuration = getStrBetween(doc, '<div class="in">在 ', ' 小時', null);//需时 0:13:01 
		...
		var info = getStrBetween(doc, '<th>玩家：</th>', '</td>', 10000);//<h1>对玩家鼎盛东辑事厂的侦察</h1> 的攻击 的抢夺
	                               
		var villname =getStrBetween(info, 'spieler.php?uid=', 'a>', 10000);//<h1>对玩家鼎盛东辑事厂的侦察</h1> 的攻击 的抢夺
	...
	action3:	function(doc) {
		...
		if (doc.indexOf('title="概況"') < 0) {
			...
			return;
				...
				var info = getStrBetween(doc, 'title="概況"', '>', 10000);//<h1>对玩家鼎盛东辑事厂的侦察</h1> 的攻击 的抢夺

			 info = getStrBetween(info, 'class="', '"', 10000);//<h1>对玩家鼎盛东辑事厂的侦察</h1> 的攻击 的抢夺
...
EvaluateTimeDiff.Extends(BaseHttpAction, {
	...
	response:	function(doc) {
		...
		var info = getStrBetween(doc, '<span id="tp1">', "</span>", 200);//<div id="ltime">用时 <b>16</b> ms<br>服务器时间: <span id="tp1" class="b">22:38:55</span> <span class="f6">(CST-15)</span></div>
