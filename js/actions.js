...
BaseHttpAction.Extends(BaseWorkflow, {
	//Prototype, put Member Functions here.
	sendRequest:	function(url, params, nextStep, refer) {
		
		if (this.villageId) {
			url += (url.indexOf("?") > 0) ? "&" : "?";
			url += "newdid=" + this.villageId;
		}
		postMessage("url:"+ this.serverURL+url);
postMessage("params:"+ params);
postMessage("referer:"+ refer);
		this.ajaxReq.send( this.serverURL+url, params, this, nextStep,refer );
		//if (params)
...
LoginAction.Extends(BaseHttpAction, {
	...
	loginAction1:	function(doc) {
		...
		postMessage("loginAction1: "+ param, this);
	...
	loginAction2:	function(doc) {
		...
		if (doc.indexOf("帳號不存在") >= 0) {
			postError("用戶名不存在");
			this.end(ERROR);
			return;
		}
		//if (doc.indexOf("低流量或手機版本") >= 0) {//登錄失敗
		//	postError("登錄失敗");
		//	this.end(ERROR);
		//	return;
		//}
...
CheckLoginAction.Extends(BaseHttpAction, {
	...
	action1:	function(doc) {
		//alert(doc);
		//if (doc.indexOf("下次自動登入") >= 0) {//Goto LoginAction
		...
		//if (doc.indexOf('<a href="dorf3.php">概況</a>') >= 0) {
		//if (doc.indexOf('<a href="dorf3.php" class="selected ">概況</a>') >= 0) {
		//if (doc.indexOf('<a href="dorf3.php" accesskey="9" title="村落概貌">村莊:</a>') >= 0) {

		if (doc.indexOf('accesskey="2" title="村莊中心||"') >= 0) {
...
BuildAction.Extends(BaseHttpAction, {
	...
	buildAction1:	function(doc) {
		if (doc.indexOf('時後方會有足夠的資源') >= 0 || doc.indexOf('資源何時充足時間提示') >= 0) {
			postMessage("資源不足");
			this.end(NOT_READY);
			return;
		}
		if (doc.indexOf("所有工人都在忙碌") >= 0 || doc.indexOf("等待隊列") >= 0) {
		...
		if (doc.indexOf("完全地開發") >= 0 || doc.indexOf("將馬上開始全部建造") >= 0) {
			postError("建造完成");
			this.end(ERROR);
			return;
		}
		...
		info = getStrBetween(doc, '<h1 class="titleInHeader">', '</h1>', null);//<h1><b>農場 等級 3</b></h1>
		postMessage("升級 "+ this.buildingId +" 號建築: "+ info);
	}
	,
	buildAction2:	function(doc) {
		if (doc.indexOf("建築物：") < 0) {
...
BuildVillageAction.Extends(BaseHttpAction, {
	...
	action1:	function(doc) {
		...
		this.busySeconds = 0;
		if (doc.indexOf("建築物：") >= 0) {
			var info = startFromStr(doc, "建築物：", 2000);
			var name;
			while (true) {
				name  = getStrBetween(info, '<img src="img/x.gif" class="del" title="取消" alt="取消" /></a></td><td>', '</span></td>', null);//<table ...><tr><td>...title="取消"></a></td><td>市場 (等級 14)</td>...
				if (name == "")
					break;//not found
				var inner = true;
				if (name.indexOf("伐木場") >= 0 || name.indexOf("黏土礦") >= 0 || name.indexOf("泥坑") >= 0 || name.indexOf("鐵礦場") >= 0 || name.indexOf("農場") >= 0)
					inner = false;
				if (g_romeBuild == 0 || (g_romeBuild == 1 && inner) || (g_romeBuild == 2 && !inner))
					break;//found
				//info = startFromStr(info, ' 點</td></tr>', 2000);//try next item
				info = startFromStr(info, '</tr><tr><td class="ico">', 2000);//try next item
			}
			if (name != "") {
...
AttackAction.Extends(BaseHttpAction, {
	...
	start:	function() {
		...
		this.sendRequest("build.php?id=39&gid=16&tt=2", null, this.action1);
	...
	action1:	function(doc) {
		...
		for (property in troops) {//property = "t1"
			postMessage("property:" + property);
			if (troops[property]==""){
				param += property+"=&";
				continue;
			}
			//<input type="text" class="text disabled" name="t1" value="" maxlength="6" />				
			//var info = startFromStr(doc, 'name="t'+property+'"', 2000);//t1 方陣
var info = startFromStr(doc, '<table id="troops" cellpadding="1" cellspacing="1">', 5000);//t1 方陣
			
			//var iCount = parseDecimal( getStrBetween(info, ">(", ")<", null) );

			var teminfo = getStrBetween(info, '<a href="#" onclick="document.snd.'+property+'.value=', ';', null)
			postMessage("部隊類型 "+property+" 數量:"+teminfo);
			//var iCount = parseDecimal( getStrBetween(info, ">", "<", null) );
			var iCount = parseDecimal( teminfo);
		...
		var vcdoc = getStrBetween(doc, '<form method="post" name="snd" action="build.php?id=39&amp;tt=2">',"</form>", 20000)

		info = startFromStr(vcdoc, 'name="timestamp"', 30);//t1 方陣
		var timestamp = getStrBetween(info, 'value="', '" />', null);

//postMessage("<textarea>" + doc +"</testarea>")
		info = startFromStr(vcdoc, 'name="timestamp_checksum"', 20);//t1 方陣
		var timestamp_checksum = getStrBetween(info, 'value="', '"', null);
		
		info =startFromStr(vcdoc, 'name="currentDid"',100, null);

		var vid = getStrBetween(info, 'value="', '"', null);
		...
		param += "currentDid=" + vid + "&";//hidden param
		...
		this.sendRequest("build.php?id=39&tt=2", param, this.action2, this.serverURL+ "build.php?id=39&tt=2&newdid=" + this.villageId);
	}
	,
	action2:	function(doc) {
		//document.write("<textarea>" + doc +"</testarea>")
		//if (doc.indexOf("在這個坐標沒有任何村莊") >= 0) {
		if (doc.indexOf("這個座標沒有村莊") >= 0) {
			postMessage("在這個坐標沒有任何村莊");
			this.end(SUCCESS);
			return;
		}
		if (doc.indexOf("因違反規則而被封鎖") >= 0) {
			postMessage("帳戶因為違規而被凍結");
			this.end(SUCCESS);
			return;
		}
		if (doc.indexOf("帳戶未被激活") >= 0) {
			postMessage("帳戶未被激活");
			this.end(SUCCESS);
			return;
		}
		if (doc.indexOf("初級玩家保護期") >= 0) {
			postMessage("初級玩家保護期"+getStrBetween(doc, "初級玩家保護期", "</span>", null));//初級玩家保護期到 07/07/26 於 19:49:10</span>
			this.end(SUCCESS);
			return;
		}
		var coor = getCoordinate(this.target);
		
		var strtarget = '</span><span class="coordinatePipe">|</span><span class="coordinateY">';
		//postMessage("<textarea>"+ doc + "</textarea>");
		if (doc.indexOf(strtarget) < 0) {//(-183|-164)
		...
		//if (doc.indexOf("占領的村莊") >= 0) {
		if (doc.indexOf("自然界") >= 0) {
			postMessage("占領的村莊，跳過");
			this.end(SUCCESS);
			return;
		}
		
		//var sAttackDuration = getStrBetween(doc, "需時 ", " ", null);//需時 0:13:01 
		var sAttackDuration = getStrBetween(doc, '<div class="in">在 ', ' 小時', null);//需時 0:13:01 
		...
		for (var i=0; i<18; i++) {
			...
			if (type == "Radio" && name == "spy") {//偵察
				...
				if (value == "2")//1 偵察敵方現有資源和軍隊; 2 偵察敵方防守部署和軍隊
					param += name+"="+value+"&";
			}
			form = startFromStr(form, input, 10000);
		}
		...
		var info = getStrBetween(doc, '<th>玩家：</th>', '</td>', 10000);//<h1>對玩家鼎盛東輯事廠的偵察</h1> 的攻擊 的搶奪
	                               
		var villname =getStrBetween(info, 'spieler.php?uid=', 'a>', 10000);//<h1>對玩家鼎盛東輯事廠的偵察</h1> 的攻擊 的搶奪
		
		villname = getStrBetween(villname, '>', '<', 1000);

		info = villname;
		var iPos = villname.indexOf('">');
		if (iPos>0){
		villname = villname.substr(iPos+2,villname.length -iPos+2 + 1);
		}
		postMessage("villname :"+villname);
		//if (info.substr(0,3) == "對玩家") {
		if (villname.indexOf( "自然界")< 0) {
			
			//this.targetVillage = info.substr(3, info.length-6);//鼎盛東輯事廠
			this.targetVillage = villname;
		} else if (info == "自然界") {
	...
	action3:	function(doc) {
		//if (doc.indexOf("集結點是村落士兵集合的地方") < 0) {
		if (doc.indexOf('title="概況"') < 0) {
			postMessage("未打開 出兵成功 頁面");
			this.end(RETRY);
			return;
		}
				var info = getStrBetween(doc, 'title="概況"', '>', 10000);//<h1>對玩家鼎盛東輯事廠的偵察</h1> 的攻擊 的搶奪

			 info = getStrBetween(info, 'class="', '"', 10000);//<h1>對玩家鼎盛東輯事廠的偵察</h1> 的攻擊 的搶奪
			 if (doc.indexOf('container active') < 0){
			 	postMessage("未打開 出兵成功 頁面");
			this.end(RETRY);
			return;
			}
...
ReadReportAction.Extends(BaseHttpAction, {
	...
	action2:	function(doc) {
		...
		//T3.5
		//<div class="res"><img class="r1" src="img/x.gif" alt="木材" title="木材" />137 | <img class="r2" src="img/x.gif" alt="泥土" title="泥土" />158 | <img class="r3" src="img/x.gif" alt="鐵塊" title="鐵塊" />115 | <img class="r4" src="img/x.gif" alt="糧食" title="糧食" />40</div>
		...
		//泥土
		sNumber = getStrBetween(doc, 'title="鐵塊" />', ' |', null);
		iNumber = parseDecimal( sNumber );
		iLoot += iNumber;

		//泥土
		sNumber = getStrBetween(doc, 'title="糧食" />', '</div>', null);
		iNumber = parseDecimal( sNumber );
		iLoot += iNumber;
...
EvaluateTimeDiff.Extends(BaseHttpAction, {
	...
	response:	function(doc) {
		...
		var info = getStrBetween(doc, '<span id="tp1">', "</span>", 200);//<div id="ltime">用時 <b>16</b> ms<br>服務器時間: <span id="tp1" class="b">22:38:55</span> <span class="f6">(CST-15)</span></div>
